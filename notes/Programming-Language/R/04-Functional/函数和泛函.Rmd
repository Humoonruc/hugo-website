...
在函数内部，可以直接用`str_c()`将`...`合并为字符串（只有一个元素）

但也可以用`list(...)`将其转化为列表（一般是多个元素）再处理

```{r}
# 嵌套函数，按顺序依次匹配
f <- function(name, value, color) {
  print(str_c("name:", name, "; value:", value, "; color:", color))
}
g <- function(name, ...) {
  f(name, ...)
}
g(name = "a", value = 3, color = "red")
g("b", 4, "blue")
...
pwalk(list(fileNames, plots), ggsave, path = getwd() %>% str_c('/Figures/'))
