...
# 输出结果每行以"#>"开头，是为了复制粘贴代码时，结果自动被理解为注释从而不必运行
...
```{r}
# 合并一个字符向量的各元素为一个长字符串，使用参数collapse =
...
str_to_upper(dog) # 转换为全大写字母
str_to_lower(dog) # 转换为全小写字母
str_to_title(dog) # 单词首字母大写
str_to_sentence("look the quick brown dog!") # 全句首字母大写
```
...
splited <- unlist(strsplit("a.b.c", ".")); splited
# fixed=T，第2个参数为文本字符串
splited <- unlist(strsplit("a.b.c", ".", fixed = T)); splited
```
