...
abstract: |
  EK 模型是对李嘉图模型的扩展，它是一个包含现实地理特征的一般均衡模型。它为双边贸易提供了简单的结构方程，其中包含了反映绝对优势（技术水平）、比较优势（各国的技术异质程度）和地理障碍（各种贸易障碍）的参数。
  
  该文使用 1990 年 19 个 OECD 国家的制造业、价格和地理数据估计了这些参数，而后使用该模型讨论了各种问题，包括贸易利得、一国技术进步红利通过贸易扩散后的分布和关税削减的影响等。
---
...
> 证明 \@ref(eq:8)：
由于独立同分布，$N$ 维空间中的联合概率密度函数可以表示为 $N$ 个概率密度函数的积$$\begin{aligned}
...
> 证明：
$n$ 国价格指数为 $p_n=\left[\int_0^1 p_n(j)^{1-\sigma}dj\right]^{\frac{1}{1-\sigma}}$. 方括号中的统计量实际上是对商品连续统的一个积分，可以将其写成面积和的形式：将 $[0,1]$ 区间划分为 $S$ 份，计算每份的面积：$\int_0^1 p_n(j)^{1-\sigma}dj=\lim_{S \to \infty} \sum_{k=1}^{S} \left[\frac{1}{S}\cdot p_{n}(k)^{1-\sigma}\right]$
...
第 \@ref(estimate-general-equilibrium) 部分估计出的结构参数（见 Table 8[^制造业支出占比]）使我们可以进行反事实分析。由于该模型的简化[^简化]，反事实分析不应被视为具有明确含义的政策分析，但它们仍然提供了一些洞见。
...
　　将这些参数和 $\vec{w}$、$\vec{Y}$ 的实际数据代入\@ref(eq:16)、\@ref(eq:17)和\@ref(eq:20)，可解出劳动力跨部门流动条件下的 $\vec{p}^{\prime}$ 和 $\vec{L}^{\prime}$，再结合\@ref(eq:31)可计算出 $\ln \left(W^{\prime}/W\right)$，于是得到 Table 9 的左半边。

　　将这些参数和 $\vec{L}$ 的实际数据、$\vec{Y^O}$ 的计算值[^非制造业收入]代入\@ref(eq:16)、\@ref(eq:17)和\@ref(eq:21)，可解出劳动力不可跨部门流动条件下的 $\vec{p}^{\prime}$ 和 $\vec{w}^{\prime}$，再结合\@ref(eq:32)可计算出 $\ln \left(W^{\prime}/W\right)$，于是得到 Table 9 的右半边。

[^非制造业收入]: 非制造业收入用 $GDP-L*w$ 得到。其中 GDP、制造业就业人数和制造业工资都用实际数据。
...
　　本节讨论专业化问题：在劳动力可跨部门流动的前提下，地理障碍消失时，由\@ref(eq:22)知，制造业就业与 $T/w^{1+\theta\beta}$ 成正比，取决于技术水平和工资（由非贸易部门的生产率决定）；当地理障碍为无穷大时，由\@ref(eq:18)和\@ref(eq:19)知，制造业就业占劳动力禀赋的比例为固定值 $\alpha$[^制造业就业比例固定] 。这两种极端情况中，制造业就业都与地理障碍无关。

[^制造业就业比例固定]: 自给自足时，\@ref(eq:18)变为 $w_n L_n = \beta X_n$。代入\@ref(eq:19)，且有劳动力可流动时工资一致，于是 $\frac{w_n L_n}{\beta} = \frac{1-\beta}{\beta}w_n L_n+\alpha w_n \bar{L_n}$。化简得 $L_n = \alpha \bar{L_n}$，即制造业就业占劳动力禀赋的比例为固定值 $\alpha$。
...
　　当劳动力可跨部门流动时，由\@ref(eq:31)式知福利只与价格有关，因为各国都享受到了价格更低的制造业产品，故各国福利普遍改善。当劳动力不可跨部门流动时，他国福利同时还受到负收入效应的影响（因为制造业工资下降）。若一国不能降低其制造业就业规模，则该国总体福利常常会下降——日本和德国是典型。
